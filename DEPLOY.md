...
- GitHub リポジトリ `hyota-nekozenouta/favgallery`（このリポ単体。アプリ一式がリポ直下にある。
  旧名 `archive` — 2026-06-11 リポ rename。HALO 側では `projects/favgallery/` に submodule として配置）
...
2. リポジトリ `hyota-nekozenouta/favgallery` を選択
...
- デスクトップ版: `projects/favgallery/ffmpeg/bin/ffmpeg.exe`（Windows 専用同梱バイナリ・HALO 内パス）
