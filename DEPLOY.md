...
3. **Root Directory** を `projects/archive` に設定
   （`nixpacks.toml` / `Procfile` / `pyproject.toml` がこの階層にある。`src/` はその下のパッケージ。
   ここを `projects/archive/src` にすると `nixpacks.toml` が見つからずビルドが壊れる）

### 1-2. ビルド設定

Railway は Root Directory 直下の `nixpacks.toml` を自動検出して使用する（これが本番のビルド方式）。
`nixpacks.toml` の `[phases.install]` は本番の依存をインストールするので、`pyproject.toml` の
`[project] dependencies` と一致している必要がある（ズレると本番だけ ModuleNotFoundError になる）。
...
（本番の Railway は `nixpacks.toml` でビルドする）:
