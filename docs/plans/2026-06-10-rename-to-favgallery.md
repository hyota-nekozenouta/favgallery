...
- [ ] **Step 2: ベースライン確認 — 全テストが green であること**
...
- [ ] **Step 1: ディレクトリを git mv**

```bash
git mv src/xlikes_viewer src/favgallery
```

- [ ] **Step 2: `xlikes_viewer` を全ファイルで `favgallery` に一括置換**
...
- [ ] **Step 5: 古い __pycache__ / egg-info を掃除して再実行（stale バイトコード対策）**
...
- [ ] **Step 3: PyInstaller spec をリネームして exe 名を変更**
...
- [ ] **Step 3: `_env_first` ヘルパーを実装して 3 箇所を読み替え**
...
- [ ] **Step 2: cli.py のデスクトップウィンドウタイトル**
...
- [ ] **Step 3: server.py の Basic realm**

```python
                headers={"WWW-Authenticate": 'Basic realm="FavGallery"'},
```

- [ ] **Step 4: ユーザー向け "Archive" 表記の残存ゼロを確認 + テスト**
...
- [ ] **Step 1: 残存 `xlikes` の全数監査 — 意図的残置のみであること**
...
- [ ] **Step 3: version bump（リネーム = 互換性に触る minor）**
...
- [ ] **Step 5: main へマージ（push はひょーたさん / メインHALO の deploy 判断と合わせる）**
