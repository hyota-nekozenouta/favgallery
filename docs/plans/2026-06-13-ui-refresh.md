...
- [ ] **Step 2: 確認** — `uv run favgallery`（または `python scripts/demo_preview.py`）で起動し、ページが今まで通り表示される（スプライトは不可視・既存グリフは未変更）。コンソールエラーが出ないこと。
...
- [ ] **Step 2: import map に登録** — index.html の importmap `"imports"` 末尾（`"mylikes": ...` の後）にカンマ区切りで追加:
...
- [ ] **Step 3: 検証** — `node scripts/check_load.mjs && node scripts/check_modules.mjs`
...
- [ ] **Step 1: チェッカーを作成（先に書く＝以降の置換ミスを赤で捕まえる）**
...
- [ ] **Step 2: 確認** `node scripts/check_icons.mjs` → ✓ / ローカル起動で 3 要素が線アイコン表示・同期/設定が押せる。
- [ ] **Step 3: Commit** `git commit -am "feat(front): サイドバー上部アイコン(同期/設定/読込)を線アイコン化"`
...
- [ ] **Step 2: 確認** `node scripts/check_icons.mjs` → ✓
- [ ] **Step 3: Commit** `git commit -am "feat(front): サイドバー見出し/本棚アイコンを線アイコン化"`
...
- [ ] **Step 2: 確認** `node scripts/check_icons.mjs` → ✓
- [ ] **Step 3: Commit** `git commit -am "feat(front): ツールバー(メニュー/タブ/表示切替/既読/取得)を線アイコン化"`
...
- [ ] **Step 2: 確認** `node scripts/check_icons.mjs` → ✓ / ライトボックス・リーダー・設定ポップオーバーを開いて目視。
- [ ] **Step 3: Commit** `git commit -am "feat(front): ライトボックス/リーダー/設定の記号を線アイコン化"`
...
- [ ] **Step 1: import 追加** — 既存 import 群の末尾に `import { icon } from "icons";`

- [ ] **Step 2: 置換**（`♥` like-btn は 3 箇所同型 / `♥ count` は 3 箇所同型 / `📍` は 2 箇所）
...
- [ ] **Step 3: 確認** `node scripts/check_modules.mjs && node scripts/check_icons.mjs` → ✓
- [ ] **Step 4: Commit** `git commit -am "feat(front): posts.js のグリフを線アイコン化"`

### Task 10: lightbox.js（いいねボタン・件数・処理中）

**Files:** Modify `src/favgallery/static/lib/lightbox.js`（冒頭 import / L63,84,85,125）

- [ ] **Step 1: import 追加** — `import { icon } from "icons";`
...
- [ ] **Step 3: 確認** `node scripts/check_modules.mjs && node scripts/check_icons.mjs` → ✓ / ライトボックスで heart/eye と処理中表示を目視。
- [ ] **Step 4: Commit** `git commit -am "feat(front): lightbox.js のグリフを線アイコン化(×乗算は保持)"`
...
- [ ] **Step 3: 確認** `node scripts/check_modules.mjs && node scripts/check_icons.mjs` → ✓ / リスト削除・フィルタチップ・作者/タグの折りたたみ開閉を目視（chevron が回る）。
- [ ] **Step 4: Commit** `git commit -am "feat(front): library.js のグリフ/トグルを線アイコン化"`
...
- [ ] **Step 1: import 追加** — 両ファイル冒頭に `import { icon } from "icons";`
...
- [ ] **Step 3: 確認** `node scripts/check_modules.mjs && node scripts/check_icons.mjs` → ✓
- [ ] **Step 4: Commit** `git commit -am "feat(front): bookshelf.js/cookies.js のグリフを線アイコン化"`
...
- [ ] **Step 3: 確認** `node scripts/check_modules.mjs && node scripts/check_icons.mjs` → ✓ / 表示切替ボタンを押してアイコンが切り替わる。
- [ ] **Step 4: Commit** `git commit -am "feat(front): main.js 表示切替アイコンの動的スワップ"`
...
- [ ] **Step 1: import 追加** — timeline.js と mylikes.js 冒頭に `import { icon } from "icons";`（sync.js は後述の通り icon 不使用なら不要）

- [ ] **Step 2: timeline.js — アイコンとラベルを分離して更新**（Task 7 で `#timelineRefreshIcon` と `#timelineRefreshLabel` を用意済み）
...
- [ ] **Step 4: sync.js — 砂時計フリップを CSS 回転ローダーへ簡素化**
...
- [ ] **Step 5: 確認** `node scripts/check_modules.mjs && node scripts/check_icons.mjs && node scripts/check_js.mjs` → ✓ / 同期・取得を実行してローダー回転と「取得中…」表示を目視。
- [ ] **Step 6: Commit** `git commit -am "feat(front): timeline/mylikes/sync のローダー・取得表示を線アイコン化"`

### Task 15: 残グリフの全消し確認（回帰ガード）

**Files:** 走査のみ

- [ ] **Step 1: 対象グリフが残っていないか全走査**
...
- [ ] **Step 2: `list-star`（黄色 ⭐ 表示）の有無を確認** — `rg -n "list-star" src/favgallery/static` で発生源を特定。グリフ ⭐ をテキストで置いている場合は `${icon('star')}` に置換し、`.list-star` の `color:#eab308` を `color: var(--ink)` へ（無彩色化）。該当なしならスキップ。

- [ ] **Step 3: 全チェック緑** `node scripts/check_js.mjs && node scripts/check_load.mjs && node scripts/check_modules.mjs && node scripts/check_icons.mjs && uv run pytest -q`
- [ ] **Step 4: Commit**（修正があれば）`git commit -am "fix(front): 残グリフ・色付き星の無彩色化"`
...
- [ ] **Step 1:** 既存 `:focus-visible`（L39）はグローバル定義済み。`.icon-btn` にもフォーカス時の視認性を担保するため、`.icon-btn:focus-visible { background:#1d1f23; color:var(--ink); }` を `.icon-btn:hover` の直後に追加。
- [ ] **Step 2:** CSS 再生成（共通コマンド）。
- [ ] **Step 3:** 確認 — キーボード Tab で各アイコンボタンにリングが出る。
- [ ] **Step 4: Commit** `git commit -am "feat(front): アイコンボタンの focus-visible を統一"`
...
- [ ] **Step 2:** CSS 再生成。
- [ ] **Step 3:** 確認 — PC でタブが「アイコン＋ラベル」、スマホ幅でアイコンのみ・選択中タブだけ差し色（自動）。
- [ ] **Step 4: Commit** `git commit -am "feat(front): タブを PC でアイコン+ラベル表示に(承認モック準拠)"`
...
- [ ] **Step 1:** `@media (pointer: coarse)` で `.icon-btn` を 44px 化（Task 2 で実施済み）。加えて、サイドバーのリスト/作者行（`.s-row` 相当の実セレクタ）に十分な行高があるか確認し、必要なら `@media (max-width: 767px)` 内で行の `padding` を増やす。具体セレクタは実装時に index.html のサイドバー行クラスを確認して対象を決める（`#listSidebar`/`#authorList` の行）。
- [ ] **Step 2:** ライトボックスのナビ（`.lb-nav-btn` 46px）・閉じる（28px アイコン）はタッチで十分。`@media (pointer: coarse)` で `.list-del`（×）のヒット領域を `min-width:44px;min-height:44px` に拡大。
- [ ] **Step 3:** CSS 再生成。
- [ ] **Step 4:** 確認 — スマホ実機 or DevTools デバイスモードでタブ・アイコンボタン・リスト行・削除が押しやすい。
- [ ] **Step 5: Commit** `git commit -am "feat(front): スマホのタップ領域・サイドバー行高を調整"`
...
- [ ] **Step 2: CSS 最終再生成**（input.css 変更が漏れなく反映されているか）
```bash
npx --yes tailwindcss@3.4.17 -c tailwind.config.js -i scripts/tailwind.input.css -o src/favgallery/static/style.css --minify
git diff --stat src/favgallery/static/style.css
```

- [ ] **Step 3: 目視スモーク** — `docs/smoke-checklist.md` に沿って PC + スマホで全アイコン・タブ切替・ライトボックス・リーダー・設定・同期/取得・リスト操作・折りたたみを確認。色付きが残っていない／無彩色＋差し色のみであること。

### Task 20: バージョン bump + デプロイ

- [ ] **Step 1: SemVer 判定** — 見た目の刷新（機能維持）につき **MINOR**。`pyproject.toml` の version を `0.4.7` → `0.5.0` に。
- [ ] **Step 2: プロダクト spec 同期** — `.company/products/projects/personal/favgallery.md` の version 記述・マイルストーン（M5: UI 刷新）を更新（メインHALO 側 / halo-version-bump 連動。dev-HALO は report で通知）。
- [ ] **Step 3: Commit**
```bash
git add pyproject.toml
git commit -m "chore(version): 0.5.0 — UI 刷新(アイコン統一/無彩色+差し色/スマホ最適化)"
```
- [ ] **Step 4: デプロイ** — GitHub main へ push（Railway 自動デプロイ）。`curl -sI https://favgallery.hyota.cloud` の `X-App-Version` が `0.5.0` になるのを確認。本番でアイコン表示・キャッシュバスト（`?v=` 資産ハッシュ更新）を確認。
